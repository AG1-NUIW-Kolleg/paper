...
# Prestretch configuration
# -------------------------
# Prestretch is specified as Engineering Strain: ε = (L − L₀) / L₀
#
# where:
#   L₀ = unloaded muscle length (rest length) = 15 cm
#   L  = stretched muscle length
#   ε  = engineering strain (dimensionless)
#
# To set prestretch, use command line argument:
#   ./muscle_with_prestretch ../settings_muscle_with_prestretch.py <strain>
#
# Examples:
#   0.0   → no elongation (L = 15.0 cm)
#   0.1   → 10% elongation (L = 16.5 cm)
#   0.5   → 50% elongation (L = 22.5 cm)

prestretch_strain = 0.067  # Default: 6.7% elongation (~1 cm stretch for 15 cm muscle)
...
muscle_extent = [3.468905, 3.468905, 35.035940]  # [cm, cm, cm] - Vol=421.6 cm^3, ratio=10.1000
n_elements_muscle = [4, 4, 10] # linear elements. each qudaratic element uses the combined nodes of 8 linear elements 8 to 4, 20 to 10
n_points_whole_fiber = 30 #60 to 30
n_fibers_x = 3 #6 to 3
n_fibers_y = 3 # 6 to 3

muscle_offset = [0.0, 0.0, 0.0]

rho = 10.575   ## [1e-4 kg/cm^3] density
...
Am = 500                          # surface area to volume ratio [cm^-1]
...
diffusion_prefactor = 3.828 / (500 * 0.58)  # Conductivity / (Am * Cm)
