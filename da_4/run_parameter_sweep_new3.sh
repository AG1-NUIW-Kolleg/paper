...
ENDTIME_MS=150  # 150 ms simulation time
...
run_simulation_wrapper() {
    ...
    if run_simulation "${force_actual}" "${exp_dir}" "${force_target}" "${aspect_ratio}" "${volume}" "${am}" "${rho}"; then
        ...
        if [[ -f "${prestretch_file}" ]] && [[ -f "${contraction_file}" ]]; then
            ...
            echo "${force_target},${force_actual},${aspect_ratio},${muscle_b},${muscle_a},${volume},${am},${rho},${initial_length},${prestretch_final},${prestretch_elongation},${prestretch_strain},${max_length},${min_length},${rom},$(date)" >> "${SUMMARY_FILE}"
        fi
...
main() {
    ...
    if [[ "${RESUME_MODE}" == "true" ]]; then
        ...
        local remaining=$((total_sims - completed_count))b_a,muscle_b_cm,muscle_a_cm,Volume_cm3,Am,Rho,Initial_Length_cm,Prestretch_Final_cm,Prestretch_Elongation_cm,Prestretch_Strain,Max_Length_cm,Min_Length_cm,ROM_cm
